...
@section('content')
<div class="space-y-8" x-data="{ editingCategory: null }">
    <!-- Page Header -->
    <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h2 class="text-2xl font-heading font-black text-gray-900 tracking-tight">{{ __('expenses.categories_title') }}</h2>
            <p class="text-gray-500 text-sm">{{ __('expenses.categories_subtitle') }}</p>
        </div>
        
        <div class="flex items-center gap-4">
            <a href="{{ route('expenses.index') }}" class="text-xs font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-colors">
                {{ __('expenses.back_to_expenses') }}
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 space-y-6">
                ...
                <form action="{{ route('expenses.categories.store') }}" method="POST" class="space-y-6">
                    @csrf
                    <div class="space-y-2">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                ...
                                <form x-show="editingCategory === {{ $category->id }}" action="{{ route('expenses.categories.update', $category) }}" method="POST" class="flex gap-2">
                                    @csrf @method('PUT')
                            ...
                            <td class="px-8 py-6 text-end">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('expenses.categories.destroy', $category) }}" method="POST" class="inline"
                                          @submit.prevent="askConfirm('{{ __('messages.delete') }}', '{{ __('expenses.category_delete_confirm', ['name' => $category->name]) }}', () => $el.submit())">
                                        @csrf @method('DELETE')
