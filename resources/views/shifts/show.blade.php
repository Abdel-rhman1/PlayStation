...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-heading font-black text-gray-900 tracking-tight">Shift Summary</h2>
                <p class="text-gray-500 mt-1 font-medium italic">Shift #{{ $shift->id }} ({{ $shift->status }})</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
            <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Opening Balance</p>
        ...
        <div class="bg-gray-900 rounded-[2rem] p-8 shadow-xl text-white">
            <p class="text-[10px] text-white/40 font-black uppercase tracking-widest mb-1">Expected Revenue</p>
            <p class="text-3xl font-heading font-black text-primary-400">+${{ number_format($summary['total_revenue'], 2) }}</p>
        </div>
        <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
            <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Expenses Logged</p>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8 border-b border-gray-50 flex items-center justify-between">
            <h3 class="text-lg font-bold text-gray-900 tracking-tight">Financial Reconciliation</h3>
            <span class="text-[10px] font-black {{ $summary['difference'] == 0 ? 'text-green-500' : 'text-red-500' }} uppercase tracking-widest px-3 py-1 bg-gray-50 rounded-lg">
                Status: {{ $summary['difference'] == 0 ? 'Balanced' : 'Discrepancy' }}
            </span>
        </div>
        
        <div class="p-8 space-y-4">
            <div class="flex justify-between items-center py-2">
                <span class="text-gray-500 font-medium">Session-based Revenue</span>
                <span class="font-black text-gray-900">${{ number_format($summary['session_revenue'], 2) }}</span>
            </div>
            <div class="flex justify-between items-center py-2">
                <span class="text-gray-500 font-medium">Direct Retail / POS Sales</span>
                <span class="font-black text-gray-900">${{ number_format($summary['order_revenue'], 2) }}</span>
            </div>
            <hr class="border-gray-50">
            <div class="flex justify-between items-center py-2">
                <span class="text-gray-900 font-black uppercase text-xs tracking-widest">Expected Cash in Drawer</span>
            ...
            <div class="flex justify-between items-center py-4 bg-primary-50 rounded-2xl px-6">
                <span class="text-primary-800 font-black uppercase text-xs tracking-widest">Employee Counted Total</span>
            ...
            <div class="p-4 rounded-xl {{ $summary['difference'] > 0 ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }} text-sm font-bold flex items-center gap-3">
                ...
                Discrepancy: ${{ number_format($summary['difference'], 2) }} ({{ $summary['difference'] > 0 ? 'Surplus' : 'Deficit' }})
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-gray-50 rounded-[2rem] p-6 border border-gray-200">
        <h3 class="text-xs font-black text-gray-500 uppercase tracking-widest mb-4">Audit Details</h3>
        <div class="grid grid-cols-2 gap-8">
            <div class="space-y-1">
                <p class="text-[10px] text-gray-400 font-black uppercase tracking-tighter">Opened By</p>
            ...
            <div class="space-y-1">
                <p class="text-[10px] text-gray-400 font-black uppercase tracking-tighter">Closed At</p>
                <p class="text-sm font-bold text-gray-800">{{ $shift->end_time ? $shift->end_time->format('H:i:s') : 'Still Open' }}</p>
                <p class="text-[10px] text-gray-400">{{ $shift->end_time ? $shift->end_time->diffForHumans($shift->start_time) : '-' }} Duration</p>
            </div>
        </div>
    </div>
</div>
